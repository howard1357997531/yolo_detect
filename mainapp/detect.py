...
def parse_opt():
    ...
    return opt
'''
讀: https://haosquare.com/python-argparse/
輸入: python test.py hello world 3
args = parser.parse_args()
print(type(args))
<class 'argparse.Namespace'>  # args 是 Namespace 物件，只是個極簡單的類別
print(args)
...
在 add_argument() 內設定 nargs 參數，可以限制你的引數有幾個：
    ...
    當你設定 nargs 為 ? 或者 * 時，表示你的程式可以接受使用者不輸入該項引數，
    此時你可以在 default 設定預設值、在使用者沒有輸入該引數時採用。

action='store_true'
只要使用者輸入此命令列引數，parse_args() 就紀錄該引數值為 True，否則為 False，
