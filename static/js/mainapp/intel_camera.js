...
$(document).ready(function () {
    ...
    $('.web-camera .ratio-detail #4-3btn').click(function () { 
        $('.web-camera .wh').removeAttr('style');
        $('.web-camera .wh').removeClass('wh16-9 wh-max');
    ...
    $('#web-camera-openbtn').click(function () { 
        ...
        $.ajax({
            type: "GET",
            url: "/check_intel_camera_isopen",
            success: function (response) {
                console.log(response.status)
                if (response.status === 'done') {
                    $('.yolo-output').append(`<img src="/intel_camera_video_feed/" alt=""></img>`);
                    ...
                    setTimeout(() => {
                        if (localStorage.getItem('language') === 'zh') {
                            ...
                            Swal.fire({
                                icon: 'error',
                                title: 'Intel Camera',
                                text: 'did not connect',
                              })
                        }
                        loading_icon.style.display = 'none'
                        loading_text.style.display = 'none'
                        camera_openbtn.removeAttribute('disabled')
